...
spec = importlib.util.spec_from_file_location("coral_reef", ROOT / "flowing_coral_reef_generator.py")
addon = importlib.util.module_from_spec(spec)
spec.loader.exec_module(addon)
addon.register()
scene = bpy.context.scene
s = scene.crf_settings
...
def assert_connected_closed(obj):
    ...
    # Closed topology alone does not detect a folded inner offset surface.
    tree = BVHTree.FromBMesh(bm)
    bm.faces.ensure_lookup_table()
    for a, b in tree.overlap(tree):
        if a < b and not set(bm.faces[a].verts).intersection(bm.faces[b].verts):
            raise AssertionError(('Self intersection', obj.name, a, b))
...
assert s.density == bpy.types.Scene.bl_rna.properties['crf_settings'].fixed_type.properties['density'].default
...
manifest = json.loads(root["crf_manifest"])
...
old_manifest = root['crf_manifest']
...
for view in ('TOP', 'ANGLED'):
    ...
    assert len([o for o in preview.objects if o.type == 'MESH' and not o.get('crf_render_helper')]) == len(objects)
    for obj in objects:
...
s.block_bevel = 0.0
assert bpy.ops.crf.generate_finished_work() == {'FINISHED'}
assert len([c for c in bpy.data.collections if c.get('crf_generated')]) == 1
objects = addon._artwork_objects(bpy.context)
for obj in objects:
    assert_connected_closed(obj)
...
for mode in ('COMMON_PANEL', 'INDIVIDUAL_FEET'):
    ...
    s.density = 12
    s.form_style = 'ORGANIC_FUNNELS'
    assert bpy.ops.crf.generate_field() == {'FINISHED'}
    assert len(addon._artwork_objects(bpy.context)) == 1
print('WORKFLOW_TESTS_PASSED', bpy.app.version_string)

# Dedicated kernel checks: determinism, stream tangent, thickness, self crossings,
# XY collision containment and all presets at a small representative scale.
from mathutils.bvhtree import BVHTree
addon._apply_style_preset(s, 'PARAGAMI_CORAL')
s.finished_width, s.finished_height = 240, 180
config = addon._master_settings(s)
specs = addon._make_specs(config)
again = addon._make_specs(config)
assert specs == again, "Same seed/settings must reproduce identical specifications"
assert any(p['river'] > .95 for p in specs) and any(p['river'] < .1 for p in specs)
for p in specs:
    verts, faces, _ = addon._build_piece_mesh_data(p, config)
    bm = bmesh.new()
    for v in verts:
        bm.verts.new(v)
    ...
    bm.faces.ensure_lookup_table()
    for a, b in tree.overlap(tree):
        if a >= b or set(bm.faces[a].verts).intersection(bm.faces[b].verts):
            continue
        raise AssertionError(('Self intersection', p['index'], a, b))
    bm.free()
    n, levels = config.radial_segments, config.vertical_segments
    start_inner = (levels+1)*n
    for k in range(1, levels+1):
        for j in range(n):
            delta = Vector(verts[k*n+j])-Vector(verts[start_inner+k*n+j])
            assert abs(delta.length-config.wall_thickness) < 1e-4
    for v in verts:
        # Include normal offset and rolled crown in the collision test.
        for nx, ny, d in p['planes']:
            assert nx*v[0]+ny*v[1] <= d + 1e-4, ('Cell escape', p['index'], v, nx, ny, d)
    # Stream direction is tangent to the same scalar field driving size.
    x, y = p['global_x'], p['global_y']
    island, angle = addon._reef_field(x, y, config)
    eps = .01
    a = addon._reef_stream(x+eps*math.cos(angle), y+eps*math.sin(angle), config)
    b = addon._reef_stream(x-eps*math.cos(angle), y-eps*math.sin(angle), config)
    assert abs(a-b) < 1e-6
print('REEF_KERNEL_OK', len(specs), 'pieces; nominal offsets, self intersections, convex containment, determinism, stream tangents')
for preset in ('PARAGAMI_CORAL', 'DENSE_MOSAIC', 'DEEP_HERO', 'STRONG_CURRENT', 'ROUNDED_BLOCKS', 'MIXED_GEOMETRY'):
    addon._apply_style_preset(s, preset)
    s.finished_width, s.finished_height = 160, 140
    assert bpy.ops.crf.generate_finished_work() == {'FINISHED'}, preset
    for obj in addon._artwork_objects(bpy.context):
        assert_connected_closed(obj)
print('ALL_REEF_TESTS_PASSED', bpy.app.version_string)
addon.unregister()
